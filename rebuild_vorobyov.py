...
REC =  [0, 1500, 1500, 1800, 2200, 3000, 3200, 2500, 2000, 3500, 4500, 5000]
...
def main():
    ...
    NEW_5_1_TEXT = (
        ...
        'оформлению самозанятости и размещению объявлений на Авито. Со второго месяца начинается '
        'активная работа: в июне–августе спрос максимален (8–11 заказов). С сентября идёт спад, '
        'структура смещается на закладку трасс в новостройках. Январь — самый низкий месяц (3 заказа), '
        'но кассовых разрывов нет. С марта — второй пик (ТО, демонтаж).'
    )
    ...
    NEW_5_2_TEXT = (
        ...
        'Расходы полностью покрываются средствами социального контракта (350 000 руб.) и собственными вложениями (19 896 руб.), '
        'кассового разрыва нет. Со второго месяца расходные материалы и транспорт растут пропорционально заказам, '
        'НПД 4% уплачивается по факту выручки. Реклама плавающая: летом 1 500 руб., в предсезонье до 5 000 руб.'
    )
    ...
    # Замена СОЛОТЧА → Солотча во ВСЕХ параграфах
    for p in paragraphs:
        for t_elem in p.iter(NS+'t'):
            if t_elem.text and 'СОЛОТЧА' in t_elem.text:
                t_elem.text = t_elem.text.replace('СОЛОТЧА', 'Солотча')
    print('   СОЛОТЧА → Солотча (все вхождения)')

    # Адрес фактического проживания → "совпадает с адресом регистрации"
    paragraphs = get_paragraphs(root)
    for p in paragraphs:
        txt = para_text(p)
        if 'фактического проживания' in txt:
            replace_paragraph_text(p, '1.4. Адрес фактического проживания: совпадает с адресом регистрации.')
            print('   1.4 адрес факт → совпадает')
            break

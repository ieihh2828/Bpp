...
def calc_transport_month(i: int) -> int:
    """Транспорт: 1500 база + 150*заказ; округление до сотен."""
    if i == 0:
        return 1500
...
def main():
    ...
    # 5 Рентабельность по месяцам = прибыль/расходы * 100
    row15 = rows5[15]; cells15 = row15.findall(NS+'tc')
    for i in range(12):
        if exp_arr[i] == 0:
            set_cell_text(cells15[2 + i], '—')
        else:
            r_pct = profit_arr[i] / exp_arr[i] * 100
            # для мая будет около -100% — оставим как есть
            set_cell_text(cells15[2 + i], f'{r_pct:.0f}%')
    ...
    NEW_5_1_TEXT = (
        ...
        'максимален (8–11 заказов в месяц). С сентября по ноябрь идёт плавный спад вместе с завершением летнего пика; '
        'структура заказов смещается на закладку трасс в новостройках г. Рязани, где I квартал 2026 г. — пик сдачи жилья. '
        'Январь — ожидаемо самый низкий месяц после новогодних праздников (3 заказа), но кассовых разрывов это не создаёт: '
        'выручка покрывает все текущие расходы, а накопленный за летний сезон чистый поток обеспечивает запас прочности. '
        'С марта–апреля начинается второй пик предсезонного спроса (ТО, чистка, демонтаж кондиционеров с дач) — '
        'клиенты заказывают обслуживание заранее, чтобы не стоять в очереди в мае–июне. '
        'Годовая выручка прогнозируется на уровне ' + fmt_rub(annual) + ' руб., что соответствует средним показателям одобренных бизнес-планов в г. Рязани.'
    )
    ...
    NEW_5_2_TEXT = (
        ...
        'осенью и в предсезонье объём рекламы увеличивается до 3 500–5 000 руб. для удержания позиций. '
        'Постоянные ежемесячные расходы (телефон, интернет — 650 руб.) обеспечивают непрерывность связи с клиентами. '
        'В статью «непредвиденные расходы» заложены амортизация инструмента и резерв на ремонт. '
        'Аренда отсутствует — оборудование и материалы хранятся в собственном гараже автора.'
    )

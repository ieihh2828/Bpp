...
def main():
    ...
    # 2.1 — описание услуг (5 услуг клининга), отстроено от Воробьёва (другая структура)
    NEW_2_1 = (
        '2.1 Направление деятельности — оказание услуг профессиональной уборки помещений для физических и юридических лиц '
        'в г. Рязани и Рязанской области. Все работы выполняются с выездом к заказчику. Парк инструмента включает '
        'моющий экстрактор для глубокой чистки, парогенератор, промышленный пылесос для сухой и влажной уборки, а также '
        'специализированную бытовую химию, подбираемую под тип поверхности и характер загрязнения.'
    )
    ...
    # 5.1 текст
    NEW_5_1 = (
        'Первый месяц проекта (май) выручки не приносит: он отведён под закупку оборудования и расходных материалов, '
        'оформление статуса самозанятого и подготовку рекламных размещений на Авито. С июня начинается основная операционная '
        'деятельность. Летние месяцы (июнь–август) формируют ровное плато из 8–14 заказов в месяц — преимущественно генеральные '
        'уборки и работы после ремонта. Осенью спрос растёт: в этот период массово сдаются новостройки, а ноябрь и декабрь '
        'традиционно дают предновогодний пик генеральных уборок. Январь — самый слабый месяц года: длительные праздники '
        'снижают активность до 3 заказов. С февраля рынок постепенно оживает, а март–апрель приносят весенний всплеск, '
        'связанный с мойкой окон после зимы и подготовкой жилья к дачному сезону.'
    )
    ...
    # 5.2 текст
    NEW_5_2 = (
        'Расходная часть первого месяца составляет ' + fmt_rub(EQUIP_TOTAL_FIRST_MONTH) + ' руб. и связана с закупкой '
        'оборудования и стартового запаса профессиональной химии. Эта сумма полностью закрывается средствами социального '
        'контракта (350 000 руб.) и собственными вложениями (' + fmt_rub(OWN) + ' руб.), кассовый разрыв при старте отсутствует. '
        'Начиная со второго месяца расходы носят преимущественно переменный характер: затраты на химию и расходные материалы, '
        'а также транспортные расходы пропорциональны количеству выполненных заказов. Налог НПД по ставке 4% уплачивается '
        'по факту полученной выручки. Расходы на рекламу зависят от сезона: летом достаточно базового продвижения '
        '(около 1 500 руб.), в предсезонье и в предновогодний период объём продвижения увеличивается до 4 500 руб.'
    )

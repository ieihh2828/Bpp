...
ESTIMATE = [
    ...
    ('1.7', 'Набор уборочный Vileda Professional УльтраСпид Мини (комплект, 2 шт.)', '1', '12198', 'vseinstrumenti.ru', 'grant'),
    ('1.8', 'Комплект спецодежды для клининга У26 КБР (комплект, 2 шт.)', '1', '9242', 'Яндекс Маркет', 'transition'),
...
# Сумма всех позиций сметы 1.1..1.9 + 4.1 — должна совпадать с GRANT + OWN.
# 93700+134800+54044+12436+9900+28750+12198+9242+3931+5000 = 364 001
EQUIP_TOTAL_FIRST_MONTH = 364001
GRANT = 350000
OWN = 14001  # 364 001 − 350 000 = 14 001 (собственные)
...
def main():
    ...
    print('>> [#7] Риски')
    RISKS = [
        ('Снижение платежеспособности потребителей', 'средняя', 'Формирование линейки тарифов с базовым пакетом по сниженной цене и индивидуальный подбор объёма работ под бюджет клиента.'),
        ('Эпидемиологическая обстановка. Риск заболеваемости', 'низкая', 'Использование средств индивидуальной защиты и соблюдение санитарно-гигиенических требований при работе на объекте.'),
        ('Уменьшение потока клиентов (сезонность)', 'средняя', 'В период низкого сезона расширение работы с корпоративным сегментом (офисы, магазины) и привлечение постоянных клиентов на регулярную уборку по договору.'),
        ('Несоответствие ожиданиям клиента', 'средняя', 'Согласование объёма и порядка работ перед выездом, фиксация результата фотоотчётом «до/после» и работа по официальному договору.'),
        ('Болезнь', 'низкая', 'Соблюдение режима труда и отдыха, наличие финансового резерва на 1–2 месяца простоя для покрытия обязательных расходов.'),
        ('Появление конкурентов', 'средняя', 'Поддержание высокого рейтинга на Авито (4,9+) за счёт стабильного качества, оперативных ответов клиентам и публикации реальных кейсов выполненных работ.'),
        ('Поломка оборудования', 'средняя', 'Плановое техническое обслуживание основного инструмента, наличие дублирующего комплекта ключевого инвентаря и аккуратная транспортировка.'),
    ]
    t7 = tables[7]

...
def main():
    ...
    print('>> [#7] Риски')
    RISKS = [
        ('Снижение платежеспособности потребителей', 'средняя', 'Предложение базового пакета уборки по доступной цене и подбор объёма работ под бюджет клиента.'),
        ('Эпидемиологическая обстановка. Риск заболеваемости', 'низкая', 'Использование средств индивидуальной защиты и соблюдение санитарных норм на объекте.'),
        ('Уменьшение потока клиентов (сезонность)', 'средняя', 'В межсезонье работа с офисами и магазинами на регулярной уборке по договору.'),
        ('Несоответствие ожиданиям клиента', 'средняя', 'Согласование объёма работ перед выездом и фотоотчёт после уборки.'),
        ('Болезнь', 'низкая', 'Стараюсь вести здоровый образ жизни.'),
        ('Появление конкурентов', 'средняя', 'Стремиться качественно выполнять работу и удерживать постоянных клиентов.'),
        ('Поломка оборудования', 'средняя', 'Регулярный уход за оборудованием и аккуратная транспортировка, наличие резервного инвентаря.'),
    ]
    t7 = tables[7]

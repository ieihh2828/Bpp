...
def main():
    ...
    RISKS = [
        ...
        ('Уменьшение потока клиентов (сезонность)', 'высокая', 'Зимой переключаться на B2B-сегмент и поддерживающую уборку.'),
    ...
    # 2.1 — описание услуг (5 услуг клининга)
    NEW_2_1 = (
        '2.1 Услуги по профессиональной уборке квартир, домов и помещений для частных и юридических лиц. '
        'Выполнение работ осуществляется с применением профессионального оборудования (моющий экстрактор, '
        'пароочиститель, промышленный пылесос) и специализированной химии для безопасной обработки всех типов поверхностей.'
    )
    ...
    if idx >= 0:
        new_text = (
            ...
            '1) Генеральная уборка — глубокая мойка всех поверхностей, чистка сантехники, мытьё окон, обеспыливание мебели, '
            'пылесос ковровых покрытий с использованием моющего экстрактора. '
            '2) Уборка после ремонта — удаление строительной пыли, остатков клея, краски, затирки и шпатлёвки '
            'с применением парогенератора Karcher SG 4/4, мытьё окон и подоконников, обеспыливание стен и потолков. '
            '3) Поддерживающая уборка — стандартная еженедельная или регулярная уборка квартир для постоянных клиентов. '
            '4) Мытьё окон и остекления — профессиональный комплект Unger ErgoTec для безразводной мойки '
            'оконных блоков и балконов до 4-х единиц. '
            '5) Химчистка мебели и ковров — обработка моющим экстрактором Santoemma с пеной и щёлочной химией CHEMSPEC, '
            'возможна обработка озонатором для устранения запахов.'
        )
    ...
    if idx >= 0:
        replace_paragraph_text(paragraphs[idx], '2.2 Целевая аудитория: Основная аудитория — физические лица (владельцы квартир в новостройках после ремонта, собственники на вторичном рынке, заказывающие генеральную и поддерживающую уборку). Дополнительный сегмент — небольшие коммерческие помещения (офисы, магазины, парикмахерские) для регулярной уборки по договору.')
    
    # 2.3 Конкуренция — переписываем
    NEW_KONK_LINES = [
        '2.3. Конкурентная среда, конкурентная позиция бизнеса.',
        'Анализ объявлений на Авито в категории «Уборка квартир и помещений» по г. Рязани (выборка более 120 активных предложений на май 2026 г.) показал, что рынок работает в трёх ценовых сегментах.',
        'Крупные клининговые компании ставят генеральную уборку 1-2 комнатной квартиры за 6 000–10 000 руб. и уборку после ремонта от 12 000–18 000 руб. Это премиальный сегмент: договор от юридического лица, гарантия, бригада из 2–3 человек. Слабая сторона — высокая итоговая стоимость работ за счёт офисных накладных и очередь до 7–14 дней в пиковые месяцы.',
        'Частники без профессионального оборудования занимают около 50% объявлений Авито, цена уборки от 120 руб. за кв.м (без фиксированной стоимости). Слабая сторона — отсутствие профессионального инструмента (бытовые пылесосы и средства из супермаркета), отсутствие парогенератора и экстрактора, без официального чека и письменной гарантии. Для уборки после строительства такие исполнители чаще всего непригодны.',
        'Частные мастера-самозанятые с полным комплектом профессионального оборудования — целевой сегмент проекта. Цена генеральной уборки в этом сегменте 4 500–6 500 руб., после ремонта — 10 000–15 000 руб., медиана работы соответствует ценам проекта.',
        'Конкурентная позиция проекта. Цены на уровне медианы профессионального сегмента. Ключевые преимущества: профессиональный моющий экстрактор и пароочиститель с фотоотчётом клиенту, промышленный пылесос для мокрой и сухой уборки, профессиональная щёлочная химия, официальный чек самозанятого, договор на каждую работу, письменная гарантия результата. Это позволяет занять позицию «премиум за цену середины рынка» без перехода в дорогой сегмент. Полный сравнительный анализ цен ключевых конкурентов — в Приложении 2.'
    ]
    ...
    # 5.1 текст
    NEW_5_1 = (
        'Выручка в первый месяц (май) отсутствует — этот период посвящён закупке оборудования, '
        'оформлению самозанятости и размещению объявлений на Авито. Со второго месяца начинается активная работа: '
        'летнее плато июнь-август (8-14 заказов) — генеральные и после ремонта. Осенью заказов больше: пик сдачи новостроек, '
        'предновогодний период (ноябрь-декабрь) — генеральные перед праздниками. Январь — глубокий спад '
        '(3 заказа, праздники), февраль — оживание. С марта весенний пик (мойка окон после зимы, генеральные перед дачным сезоном).'
    )
    ...
    # 5.2 текст
    NEW_5_2 = (
        'В первый месяц (май) расходы составляют ' + fmt_rub(EQUIP_TOTAL_FIRST_MONTH) + ' руб. — это закупка всего необходимого оборудования и химии. '
        'Расходы полностью покрываются средствами социального контракта (350 000 руб.) и собственными вложениями (' + fmt_rub(OWN) + ' руб.), '
        'кассового разрыва нет. Со второго месяца расходные материалы (профессиональная химия, расходники) и транспорт растут '
        'пропорционально количеству заказов, налог НПД 4% уплачивается по факту выручки. Реклама плавающая по сезону: '
        'летом 1 500 руб., в предсезонье и предновогодний пик до 4 500 руб.'
    )
    ...
    if idx >= 0:
        replace_paragraph_text(paragraphs[idx], '7. Приложение. Требуемое оборудование; конкуренты и цены')

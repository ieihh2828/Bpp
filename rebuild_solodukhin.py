...
def main():
    ...
    # 3.1. Режим работы — была двойная точка «без выходных..»
    idx = find_para_by_substr(paragraphs, '3.1. с 09:00')
    if idx < 0:
        idx = find_para_by_substr(paragraphs, 'без выходных')
    if idx >= 0:
        replace_paragraph_text(paragraphs[idx],
            '3.1. Режим работы: с 09:00 до 20:00 без выходных. Выезд на объекты осуществляется по '
            'предварительной договорённости с заказчиком.'
        )

    # 3.2. Штатное расписание — была двойная точка «не требуются..»
